import { Connection, PublicKey, ParsedConfirmedTransaction, SignatureResult } from '@solana/web3.js';
...
export class SolanaClient {
  ...
  async getParsedTransaction(signature: string): Promise<ParsedConfirmedTransaction | null> {
    this.logger.info('get tx', signature);
    const tx = await this.request<ParsedConfirmedTransaction | null>('getParsedTransaction', signature, { maxSupportedTransactionVersion: 0 });
    return tx;
  }

  async getTokenAccountsByOwner(accountAddress: string): Promise<any[]> {
    const accountPublicKey = new PublicKey(accountAddress);
    const result = await this.request<any>('getTokenAccountsByOwner', accountPublicKey, {
      programId: TOKEN_PROGRAM_ID,
    });
    return result.value;
  }

  async getFinalizedSignaturesForAddress(address: string, options?: SignatureOptions): Promise<SignatureResult[]> {
    const accountPublicKey = new PublicKey(address);
    const signatures = await this.request<SignatureResult[]>('getSignaturesForAddress', accountPublicKey, options, 'finalized');
    this.logger.info('received signatures', signatures?.length);
    return signatures;
  }

  async getTokensMetadata(mintAddress: string): Promise<TokenMetadata | null> {
    return this.requestStack.addRequest(async () => {
